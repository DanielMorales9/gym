...
public class WorkoutService implements ICrudService<Workout, Long> {
  ...
  @Autowired private WorkoutRepository repo;

  @Override
  @CachePut(
      value = "workouts-single",
      key = "#result.id",
      condition = "#result != null")
  public Workout findById(Long var1) {
    return repo.findById(var1)
        .orElseThrow(() -> new NotFoundException("Workout", var1));
  }
  ...
  public Page<Workout> search(
    ...
    if (tag != null && name != null) {
      page =
          filterAndPageWorkoutsByQueryParams(name, tag, isTemplate, pageable);
    }
  ...
  private Page<Workout> filterAndPageWorkoutsByQueryParams(
      String name, String filter, Boolean isTemplate, Pageable pageable) {
    List<Workout> workouts =
        findAll().stream()
            .filter(
                workout ->
                    (isTemplate == null) || workout.isTemplate() == isTemplate)
            .filter(workout -> workout.getName().contains(name))
            .filter(
                workout ->
                    (workout.getTag1() != null
                            && workout.getTag1().equals(filter))
                        || (workout.getTag2() != null
                            && workout.getTag2().equals(filter))
                        || (workout.getTag3() != null
  ...
  public Page<Workout> searchByNameAndTemplate(
      String query, Boolean isTemplate, Pageable pageable) {
    return repo.findByNameContainsAndIsTemplateIs(query, isTemplate, pageable);
  }

  public Page<Workout> filterByNameAndTemplate(
      String query, Boolean isTemplate, Pageable pageable) {
    return repo.findByTag1EqualsOrTag2EqualsOrTag3EqualsAndIsTemplateIs(
        query, query, query, isTemplate, pageable);
  }
  ...
  public Page<Workout> filterByName(String query, Pageable pageable) {
    return repo.findByTag1EqualsOrTag2EqualsOrTag3Equals(
