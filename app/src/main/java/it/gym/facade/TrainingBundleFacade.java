...
public class TrainingBundleFacade {
  ...
  public ATrainingBundle patchBundle(Long id, HttpServletRequest request)
      throws IOException {
    ATrainingBundle bundle = findById(id);
    bundle =
        objectMapper.readerForUpdating(bundle).readValue(request.getReader());
    if (bundle.isExpired()) {
      bundle.terminate();
    } else {
      bundle.activate();
