...
public class EventFacade {
  ...
  public List<AEvent> findAllEventsByInterval(
      Date startTime,
      Date endTime,
      ...
      Long trainerId) {
    Stream<AEvent> s =
        service.findAllEvents(startTime, endTime).stream()
          ...
          s.filter(
              c -> {
                if (PersonalTrainingEvent.TYPE.equals(c.getType())) {
                  return ((PersonalTrainingEvent) c)
                      .getReservations()
                      .get(0)
                      .getUser()
                      .getId()
          ...
          s.filter(
              c -> {
                if (TimeOff.TYPE.equals(c.getType())) {
                  return ((TimeOff) c).getUser().getId().equals(trainerId);
                } else {
                  return true;
                }
              });
  ...
  private void checkNoOtherEventsExceptMe(Event event) {
    ...
    Date endTime = event.getEndTime();
    List<AEvent> events =
        this.service.findOverlappingEvents(startTime, endTime);
  ...
  private void checkNoOtherEvents(Event event) {
    ...
    if (!events.isEmpty()) throw new BadRequestException(PRESENT_EVENTS_EX);
  }
