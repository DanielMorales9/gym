...
public class EventController {
  ...
  public ResponseEntity<EventResource> deleteCourseEvent(
      @PathVariable Long id) {
  ...
  @GetMapping
  @PreAuthorize("isAuthenticated()")
  public List<EventResource> findAllEventsByInterval(
      ...
      @RequestParam(value = "endTime")
          @DateTimeFormat(
              ...
              iso = DateTimeFormat.ISO.DATE_TIME)
          Date endTime,
      @RequestParam(value = "types") HashSet<String> types,
      @RequestParam(required = false) Long customerId,
      @RequestParam(required = false) Long trainerId) {
    List<AEvent> events =
        facade.findAllEventsByInterval(
            startTime, endTime, types, customerId, trainerId);
    return new ArrayList<>(
        new EventAssembler().toCollectionModel(events).getContent());
  }
}
