...
public interface EventStatsRepository extends JpaRepository<AEvent, String> {
    ...
    @Query(value =
            "select extract(dow from e.start_time) as dayofweek, e.type as type, av(e.type) as numreservations " +
            "from events as e left join reservations r on e.event_id = r.event_id " +
            "where e.start_time > date_trunc('month', now()) - cast(:timeInterval AS Interval) " +
            "       and (e.type = 'P' or e.type = 'C') " +
