...
public class PaymentServiceTest {
  ...
  @MockBean private PaymentRepository repository;
  ...
  public void findById() {
    Mockito.when(repository.findById(1L))
        .thenAnswer(
            invocationOnMock -> Optional.of(createPayment(1L, 10., date)));
  ...
  public void findAll() {
    Mockito.when(repository.findAll())
        .thenAnswer(
            invocationOnMock ->
                Collections.singletonList(createPayment(1L, 10., date)));
    List<Payment> u = this.service.findAll();
    assertThat(u)
        .isEqualTo(Collections.singletonList(createPayment(1L, 10., date)));
