...
public class ReservationFacadeTest {
    ...
    @MockBean private TimeOffService timeService;
    ...
    public void hasHolidays() {
        List<TimeOff> holidays = Collections.singletonList(createHoliday(createAdmin()));
    ...
    public void hasNoAvailableTrainers() {
        Mockito.doReturn(1L).when(trainerService).countAllTrainer();
        List<TimeOff> timesOff = Collections.singletonList(createTimeOff(createTrainer()));
    ...
    private TimeOff createTimeOff(Trainer trainer) {
        TimeOff time = new TimeOff();
        time.setId(2L);
        time.setName("my time");
        time.setType("trainer");
    ...
    private TimeOff createHoliday(Admin admin) {
        TimeOff time = new TimeOff();
        time.setId(1L);
        time.setName("my time");
        time.setType("admin");
