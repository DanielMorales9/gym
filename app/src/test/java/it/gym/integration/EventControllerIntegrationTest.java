...
public class EventControllerIntegrationTest extends AbstractIntegrationTest {
  ...
  public void before() {
    ...
    ATrainingBundle personalBundle =
        personalSpec.createTrainingBundle(optionId);
  ...
  public void whenFindByIdReturnsOK() throws Exception {
    ResultActions result =
        mockMvc
            .perform(get("/events/" + event.getId()))
            .andExpect(status().isOk());
  ...
  public void whenCreateHolidayReturnsException() throws Exception {
    ...
    CourseTrainingEvent courseEvent =
        createCourseEvent(1L, "course", start, end, courseSpec);
  ...
  public void whenDeleteHolidayReturnsOK() throws Exception {
    ResultActions result =
        mockMvc
            .perform(delete("/events/holiday/" + event.getId()))
            .andExpect(status().isOk());
  ...
  public void whenEditHolidayReturnsOK() throws Exception {
    ...
    ResultActions result =
        mockMvc
            .perform(
                patch("/events/" + gym.getId() + "/holiday/" + event.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(json))
            .andExpect(status().isOk());
  ...
  public void whenEditTimeOffReturnsOK() throws Exception {
    ...
    Date end = addHours(start, 1);
    TimeOff timeOff =
        (TimeOff) createTimeOff(1L, "closed", start, end, trainer);
    ...
    ResultActions result =
        mockMvc
            .perform(
                patch("/events/" + gym.getId() + "/timeOff/" + timeOff.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(json))
            .andExpect(status().isOk());
  ...
  public void whenDeleteCourseEvent_OK() throws Exception {
    ...
    CourseTrainingEvent event =
        eventRepository.save(
            createCourseEvent(1L, "course", start, end, courseSpec));

    ATrainingSession session =
        sessionRepository.save(courseBundle.createSession(event));
    ATrainingSession session1 =
        sessionRepository.save(courseBundle.createSession(event));
    ...
    Customer customer =
        createCustomer(
            1L, "test@test.com", "test", "test", "test", true, null, true);
    customer = userRepository.save(customer);

    Customer customer1 =
        createCustomer(
            2L, "test1@test1.com", "test", "test1", "test1", true, null, true);
    ...
    ResultActions result =
        mockMvc
            .perform(delete("/events/course/" + event.getId()))
            .andExpect(status().isOk());
  ...
  public void whenCompleteThenOK() throws Exception {
    ...
    event = eventRepository.save(event);
    ATrainingSession session =
        courseBundle.createSession((ATrainingEvent) event);
    courseBundle.addSession(session);

    ResultActions result =
        mockMvc
            .perform(get("/events/" + event.getId() + "/complete"))
            .andExpect(status().isOk());
